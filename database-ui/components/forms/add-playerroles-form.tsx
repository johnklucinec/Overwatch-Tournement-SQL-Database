...
import { FormSchema } from "@/components/forms/add-player-form";
import { z } from "zod";
...
export const RankRole: React.FC<RankRoleProps> = ({ form, playerRole}) => {
  ...
  const rankName = `${playerRole.toLowerCase()}Rank` as keyof z.infer<
    typeof FormSchema
  >;
  const divisionName = `${playerRole.toLowerCase()}Division` as keyof z.infer<
    typeof FormSchema
  >;
